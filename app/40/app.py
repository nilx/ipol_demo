...
for "Combined First and Second Order Total Variation Inpainting Using Split Bregman"
by K. Papafitsoros, B. Sengul, C. Schoenlieb
...
class app(base_app):
    ...
    is_test = False    
    default_param = {'alpha': 1.0e-3,  # default parameters
            'beta': 1.0e-3,
            'pensize': 5,
            'pencolor':'yellow'}
    ...
    def build(self):
        ...
        archive = 'fstv_1.3'
        tgz_url = 'http://www.ipol.im/pub/art/2013/40/' \
            + archive + '.tgz'
        tgz_file = self.dl_dir + archive + '.tgz'
        progs = ['fstv', 'random_mask']
        ...
        if all([(os.path.isfile(bin_file)
            ...
            # build the programs
            build.run('make -j4 -C %s -f Makefile %s'
    ...
    def run(self):
        ...
        if self.cfg['meta']['original']:
            ...
            ar.add_info({"alpha": '%.1e' % float(self.cfg['param']['alpha'])})
            ar.add_info({"beta": '%.1e' % float(self.cfg['param']['beta'])})
            ar.save()
    ...
    def run_algo(self, stdout=None):
        ...
        # Run tvrestore
        self.wait_proc(self.run_proc(['fstv', 
            'u0.png', 'mask.png', 'inpainted.png', 
