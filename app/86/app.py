...
class app(base_app):
    ...
    xlink_article = 'http://www.ipol.im/pub/pre/86/'
    ...
    def build(self):
        ...
        """
        zip_filename = 'blsgsm_v1.0.zip'
        src_dir_name = 'blsgsm_v1.0'
        prog_filename = 'blsgsm'
        # store common file path in variables
        tgz_file = self.dl_dir + zip_filename
        prog_file = self.bin_dir + prog_filename
        log_file = self.base_dir + "build.log"
        # get the latest source archive
        build.download('http://www.ipol.im/pub/pre/86/' + \
    ...
    def run_algo(self, sigma, timeout=False):
        """
        the core algo runner
        could also be called by a batch processor
        """
        procOptions = ['blsgsm', 'input_0.sel.png', '%d' % sigma, \
                       'denoised.png', 'noisy.png', 'diff.png', '1']
