...
from numpy import * #TODO replace( ) all call loadtxt with np.replace( )
...
def draw_matches(pairs, image1, image2, imout):
    ...
    for pair in f:
        ...
        ''' read pair '''
        pair = pair.split()
        [x1,y1,sigma1,theta1, x2,y2,sigma2,theta2] = map(float,pair[0:8])
        ...
        r = radfact*sigma1
        coord = map(int,(y1-r,x1-r,y1+r,x1+r))
        ...
        r = radfact*sigma2
        coord = map(int,(y2+ofst-r,x2-r,y2+ofst+r,x2+r))
...
def draw_one_match(pairdata, pairs, image1, image2, d,lambda_ori,lambda_descr,n_hist, imout):
    """
    ...
    [x1, y1, sigma1, theta1]  = map(float,pairdata[0:4])
    [x2a,y2a,sigma2a,theta2a] = map(float,pairdata[d:d+4])
    [x2b,y2b,sigma2b,theta2b] = map(float,pairdata[2*d:2*d+4])
...
def draw_keys_oriented(keys, image, imout):
    ...
    for key in f:
        key = key.split()
        [x,y,sigma,theta] = map(float,key[0:4])
        r = radfact*sigma
        coord = map(int,(y-r,x-r,y+r,x+r))
        draw.arc(coord,0,360,fill=(0,255,0))
...
def illustrate_keypoint(keydata,n_bins,t,n_hist,n_ori,label):
    ...
    o       = keydata[4+dim]  # octave index
    s       = keydata[4+dim+1]  # scale index
...
def countdetections(keys, n_spo):
    ...
    """
    O = map(int,loadtxt(keys,usecols=(4,)))
    S = map(int,loadtxt(keys,usecols=(5,)))
...
def plot_detection_hists(keysBefore, keysAfter, label, n_oct, n_spo):
    ...
    """
    O = map(int,loadtxt(keysBefore,usecols=(4,)))
    S = map(int,loadtxt(keysBefore,usecols=(5,)))  
    histBefore = histogram(multiply(O,n_spo)+S,n_oct*n_spo)[0]
    O = map(int,loadtxt(keysAfter,usecols=(4,)))
    S = map(int,loadtxt(keysAfter,usecols=(5,)))
...
def draw_rotatedbox2(draw,x,y,radius,angle,colorvec):
    xA = int(x + radius*( (-1)*cos(angle)-(+1)*sin(angle)) )
    yA = int(y + radius*( (-1)*sin(angle)+(+1)*cos(angle)) )
    xB = int(x + radius*( (+1)*cos(angle)-(+1)*sin(angle)) )
    yB = int(y + radius*( (+1)*sin(angle)+(+1)*cos(angle)) )
    ...
    xD = int(x + radius*( (-1)*cos(angle)-(-1)*sin(angle)) )
    yD = int(y + radius*( (-1)*sin(angle)+(-1)*cos(angle)) )
    draw.line((yA,xA,yB,xB),fill='#eeefff')
    draw.line((yB,xB,yC,xC))
    draw.line([(yC,xC),(yD,xD)])
    draw.line([(yA,xA),(yD,xD)])  
...
def draw_descriptorgridNEW(draw, x,y, sigma, ori, n_hist,lambda_descr):
    #sigma = 4*sigma
    ...
    #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO 
    n_hist = n_hist-1
    lambda_descr = 0.7 * lambda_descr
    #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO 
    #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO #TODO TODO 
    
    
    K = 4 # Nhist histograms cover uniformly a segment of width K*sigmawindow*sigma (K*sigmawindow in the keypoint referential)
    
    # rgb coordinates at corners of the descriptor grid for linear interpolation in the RGB scale
    # A-B    red    - blue
    # D-C    yellow - vert
    A = (255,0,0); B = (90,90,255);
    D = (0,255,0); C = (200,150,122);
    
    for i in range(1,1+n_hist):
        for j in range(1,1+n_hist):
            
            Xi = (i-0.5*(1+n_hist))*2*lambda_descr/float(n_hist)
            Yj = (j-0.5*(1+n_hist))*2*lambda_descr/float(n_hist)
            
            # (xij,yij) coordinates in the image referential
            xij = sigma*(cos(ori)*Xi-sin(ori)*Yj)+x
            yij = sigma*(sin(ori)*Xi+cos(ori)*Yj)+y
            # color interpolation
            alpha = (i-1)/float(n_hist)
            beta = (j-1)/float(n_hist)
            colorvec = tuple([int(round((1-alpha)*(1-beta)*A[k]+(1-alpha)*beta*B[k] + alpha*(1-beta)*C[k] + alpha*beta*D[k])) for k in range(3)])
            
            draw_rotatedbox(draw, xij, yij, 0.5*float(n_hist+1)*lambda_descr*sigma/float(n_hist) ,ori,colorvec) # non overlapping regions
    radius = 0.75*(4*sigma*lambda_descr)
    draw.line([(y,x),(y+radius*sin(ori),x+radius*cos(ori))],fill=(0,255,0))
