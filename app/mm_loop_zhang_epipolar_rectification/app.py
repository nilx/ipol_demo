...
class app(base_app):
    ...
    title = "Loop-Zhang Epipolar Rectification"
    ...
    def build(self):
        ...
        tgz_file = self.dl_dir + "LoopZhangRectification.tar.gz"
        tgz_url = "https://edit.ipol.im/edit/algo/" + \
            "mm_epipolar_rectification/LoopZhangRectification.tar.gz"
        build_dir = (self.src_dir
                     + os.path.join("LoopZhangRectification", "build")
                     + os.path.sep)
        src_bin = dict([(build_dir + os.path.join("bin", prog),
                         self.bin_dir + prog)
                        for prog in ["homography", "orsa",
                                     "rectify_mindistortion",
                                     "show_epipoles",
                                     "sift", "size", "showRect"]])
        src_bin[self.src_dir
                + os.path.join("LoopZhangRectification",
                               "scripts", "MinDistortionRectify.sh")] \
                               = self.bin_dir + "MinDistortionRectify.sh"
    ...
    def run_algo(self, timeout=None):
        ...
        p = self.run_proc(['MinDistortionRectify.sh',
