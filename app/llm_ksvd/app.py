...
K-SVD Image Denoising Algorithm"
    ...
    def build(self):
        ...
        if (os.path.isfile(prog_file)
            ...
            # build the program
            build.run("make -j4 -C %s %s" % (self.src_dir + src_dir_name,
                                             os.path.join(".", 'ksvd')),
                      stdout=log_file)
            # save into bin dir
    ...
    def params(self, newrun=False, msg=None, \
               ...
               sigma=None, speedup=None, computebias=None, dobestpsnr=None):
        """
        ...
        return self.tmpl_out("params.html",
                             ...
                             dobestpsnr=dobestpsnr)


    @cherrypy.expose
    @init_app
    def rectangle(self, action=None, sigma=None, speedup=None,
                  computebias=None, dobestpsnr=None, \
    ...
    def run(self):
        ...
        speedup = self.cfg['param']['speedup']
        computebias = self.cfg['param']['computebias']
        dobestpsnr = self.cfg['param']['dobestpsnr']

        # run the algorithm
        try:
            run_time = time.time()
            self.run_algo(sigma, speedup, computebias, dobestpsnr)
        ...
        if self.cfg['meta']['original']:
            ...
            ar.add_info({"speedup": speedup})
            ar.add_info({"dobestpsnr": dobestpsnr})
            ar.add_info({"computebias": computebias})

            ar.save()
        return self.tmpl_out("run.html")

    def run_algo(self, sigma, speedup, computebias, dobestpsnr, timeout=False):
        """
        ...
        try:
            test = str(speedup)            
            speedupParam = 1 if test != 'None' else 0
        except StandardError:
            speedupParam = 0

        try:
            test = str(dobestpsnr)            
            dobestpsnrParam = 1 if test != 'None' else 0
        except StandardError:
            dobestpsnrParam = 0
        ...
        procOptions = ['ksvd', 'input_0.sel.png', '%d' % sigma, \
                       'noisy.png', 'denoised.png', 'diff.png', \
                       'bias.png', 'diff_bias.png', \
                       '%d' % computebiasParam, '%d' % speedupParam, \
                       '%d' % dobestpsnrParam]
    ...
    def result(self):
        ...
        return self.tmpl_out("result.html", sigma=sigma, \
                             speedup=speedup, computebias=computebias, \
                             dobestpsnr=dobestpsnr, \
