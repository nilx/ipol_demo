...
class app(base_app):
    ...
    def build(self):
        ...
        src_bin = dict([(self.src_dir + os.path.join("src_demoDCTdenoisingB", prog),
                         self.bin_dir + prog)
                        for prog in progs])
        ...
        if all([(os.path.isfile(bin_file)
            ...
            build.run("make -j4 -C %s %s"
                      % (self.src_dir + "src_demoDCTdenoisingB", " ".join(progs)),
                      stdout=log_file)
            # save into bin dir
	...
	#noisy and denoised images
        p = self.run_proc(['demo_DCTdenoising', 'input_0.sel.png', str(sigma), 'input_1.png', 'output_1.png'],
