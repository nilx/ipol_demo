...
class app(base_app):
    ...
    def build(self):
        ...
        tgz_url = "http://www.ipol.im/pub/art/2011/gl_lcc/LCC.tar.gz"
        ...
        if (os.path.isfile(prog_file)
            ...
            shutil.copy(self.src_dir 
                        + os.path.join("LCC", "localcolorcorrection"), 
                        prog_file)
    ...
    @cherrypy.expose
    @init_app
    def params2(self, newrun=False, msg=None, x0=None, y0=None, 
                x1=None, y1=None, r=None):
        """
        configure the algo execution
        """
	    ...
	    #use whole image, go to the parameter page with the key
            if (r != None):
                http.redir_303(self.base_url + 
                               "params2?key=%s&r=%s" % (self.key, r))
            else:
                http.redir_303(self.base_url + 
                               "params2?key=%s" % (self.key))
            ...
            if x0 == None:
                ...
                assert (x1 - x0) > 0
                assert (y1 - y0) > 0
                # go to parameter page, with the key and the subimage corners
                if (r != None):
                    http.redir_303(self.base_url + 
                              "params2?key=%s&x0=%s&y0=%s&x1=%s&y1=%s&r=%s" % 
                              (self.key, x0, y0, x1, y1, r))
                else:
                    http.redir_303(self.base_url + 
                              "params2?key=%s&x0=%s&y0=%s&x1=%s&y1=%s" % 
                              (self.key, x0, y0, x1, y1))
            return
    ...
    def result(self):
        ...
        if (r <= rmax):
            ...
            return self.tmpl_out("result.html", r=None, x0=x0, y0=y0, 
                                 x1=x1, y1=y1, sizeY="%i" % sizeYmax)
