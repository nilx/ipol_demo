...

    ...
    def run_algo(self):
        ...
        glob_pattern = self.work_dir + "b_???.png"
        fnames = [os.path.basename(f) for f in glob.glob(glob_pattern)]
        lbtxt = ""
        for l in sorted(fnames):
                lbtxt = lbtxt + l + "\n"
        print("lbtxt =  " + lbtxt)
        fi = open(self.work_dir + "lb.txt", "w")
        fi.write(lbtxt)
        fi.close()
        for k in app.parconfig:
            print(k + ' = ' + str(self.cfg['param'][k]))
        start_time = time.time()
        p = self.run_proc(["filter_pca.exe",
                    "--list", "lb.txt",
                    "--output", "o",
                    "--window-width", str(winsize),
                    "--first-vec", str(firstvec),
                    "--last-vec", str(lastvec)
                   ])
