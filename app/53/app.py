"""
m_fds_stack_mcm ipol demo web app
"""
...
class app(base_app):
    ...
    is_test = False
    xlink_article = "http://www.ipol.im/pub/art/2013/53/"
    ...
    def build(self):
        ...
        mcm_tgz_file = self.dl_dir + "stackmcm_1.1.tar.gz"
        # ***Fix URL later***
        mcm_tgz_url = \
            "http://www.ipol.im/pub/art/2013/53/stackmcm_1.1.zip"
        mcm_prog_file = self.bin_dir + "stack"
        mcm_log = self.base_dir + "build.log"
        ...
        if (os.path.isfile(mcm_prog_file)
            ...
            build.run("make -j4 -C %s"
                      % (self.src_dir + os.path.join("stackmcm_1.1", "stack_mcm")),
                      stdout=mcm_log)
            # save into bin dir
            shutil.copy(self.src_dir + os.path.join("stackmcm_1.1", "stack_mcm", "stack"),
                        mcm_prog_file)
        ...
        mcm_tgz_url = \
            "http://www.ipol.im/pub/art/2013/53/fds_mcm.tar.gz"
    ...
    def run(self):
        ...
        if self.cfg['meta']['original']:
            ...
            ar.add_file("output_Stack.png", info="output Stack")
    ...
    def run_algo(self, scale_r, stdout=None, timeout=False):
        ...
        stdout = stdout
        # process image
        p1 = self.run_proc(['stack', str(scale_r),
                            self.work_dir + 'input' + self.input_ext,
                            self.work_dir + 'output_Stack' + self.input_ext])
        p2 = self.run_proc(['mcm', str(scale_r),
                            self.work_dir + 'input' + self.input_ext,
                            self.work_dir + 'output_MCM' + self.input_ext])
        self.wait_proc([p1, p2], timeout)
        im = image(self.work_dir + 'output_Stack' + self.input_ext)
        im.save(self.work_dir + 'output_Stack.png')
